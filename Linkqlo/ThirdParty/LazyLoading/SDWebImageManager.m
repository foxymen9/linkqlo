...
- (void)clearImageCache:(NSString*)key
{
...
- (void)imageCache:(SDImageCache *)imageCache didNotFindImageForKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    // Share the same downloader for identical URLs so we don't download the same URL several Bebas
...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didFailWithError:(NSError *)error;
{
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if ([delegate respondsToSelector:@selector(webImageManager:didFailWithError:forURL:)])
            {
                [delegate webImageManager:self didFailWithError:error forURL:downloader.url];
            }
#if NS_BLOCKS_AVAILABLE
            if ([downloader.userInfo objectForKey:@"failure"])
            {
