...
@interface TryToFollowViewController ()<UserTableViewCellDelegate>
{
...
@implementation TryToFollowViewController
...
- (void) viewWillAppear:(BOOL)animated
{
    ...
    [self performSelector:@selector(_initView) withObject:nil afterDelay:0.1];
}
...
{
//    [self performSegueWithIdentifier:@"whatsnew" sender:nil];
...
- (void) followWithView:(UIView *)view
{
    ...
    if (followerInfo != nil)
    {
        ...
        {
            if ([self followFrield:followerInfo follow:NO])
            {
...
//                [self updateList];
        ...
        {
            if ([self followFrield:followerInfo follow:YES])
            {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *strUserID = [user objectForKey:@"user_id"];
    if (strUserID == nil || ![strUserID isKindOfClass:[NSString class]] || strUserID.length == 0)
        return;
